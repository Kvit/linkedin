...
a contact in `extracted` / `analysis` so the two can be analysed together.
    ...
    uv run python messages_sync.py                  # incremental, since 2023-01-01
    uv run python messages_sync.py --dry-run        # read everything, write nothing
    uv run python messages_sync.py --since 2023-01-01   # widen the history
    uv run python messages_sync.py --verify         # check for gaps
    uv run python messages_sync.py --no-contact-stats   # skip the analysis roll-up
